  ...
  const fetchStudentRoom = async () => {
    try {
      ...
      if (allocation && allocation.rooms && 'room_number' in allocation.rooms) {
        setRoomNumber(allocation.rooms.room_number as string);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (submitError) {
        ...
        throw new Error(submitError.message || 'Failed to submit laundry request');
      }
      ...
      setError('Failed to submit laundry request');
    } finally {
      setSubmitting(false);
    }
  };
